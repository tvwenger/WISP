...
def makePB(
    ...
    nchan = shp[3]
    start = qa.quantity(
        csysa.referencevalue()["numeric"][3], csysa.units()[3]
...
class Imaging:
    ...
    def mfs_dirty_cont(self):
        ...
        casa.exportfits(
            ...
            fitsimage="{0}.dirty.pbcor.image.alpha.error.fits".format(
                imagename
            ),
            overwrite=True,
            history=False,
    ...
    def mfs_clean_cont(self):
        ...
        if not self.interactive:
            ...
            dat = casa.imstat(
                imagename="{0}.residual.tt0".format(
                    os.path.join(self.outdir, imagename)
                ),
                axes=[0, 1],
                mask="'{0}.mask' == 0".format(
                    os.path.join(self.outdir, imagename)
                ),
            )
            self.logger.info(
            ...
            self.logger.info(
                "Using max MAD*1.4826 times {0} (user-defined) as threshold".format(
                    self.cp["nrms"]
                )
            )
            threshold = "{0:.2f}mJy".format(
                self.cp["nrms"] * 1000.0 * 1.4826 * np.max(dat["medabsdevmed"])
            )
        else:
            #
        ...
        casa.exportfits(
            imagename="{0}.pbcor.image.alpha.error".format(imagename),
            fitsimage="{0}.clean.pbcor.image.alpha.error.fits".format(
                imagename
            ),
            overwrite=True,
            history=False,
    ...
    def mfs_dirty_spws(self, spws):
        ...
        for spw in spws.split(","):
            ...
            # Dirty image
            #
            imagename = "{0}.spw{1}.{2}.mfs".format(
                self.field, spw, self.stokes
            )
            if self.uvtaper:
                imagename = imagename + ".uvtaper"
            imagename = os.path.join(self.outdir, imagename)
        ...
        for spw in spws.split(","):
            ...
            if not self.interactive:
                ...
                if self.savemodel == "light":
                    savemodel = "modelcolumn"
                self.logger.info(
                    "Lightly cleaning spw {0} (MFS)...".format(spw)
                )
                casa.tclean(
                    vis=self.vis,
                    imagename=imagename,
    ...
    def channel_dirty_spws(self, spws, spwtype):
        ...
        if spwtype == "cont":
            ...
            spw_inds = [
                self.config.get("Spectral Windows", "Line")
                .split(",")
                .index(spw)
                for spw in spws.split(",")
            ]
            restfreqs = [
                self.config.get("Clean", "restfreqs").split(",")[spw_ind]
            ...
            if self.cp["start"] and self.cp["end"]:
                ...
                if self.cp["width"]:
                    nchans = [
                        int(
                            abs(
                                float(self.cp["end"]) - float(self.cp["start"])
                            )
                            / float(self.cp["width"])
                        )
                        + 1
            ...
            self.logger.critical(
                "Error: spwtype {0} not supported".format(spwtype)
            )
        ...
        for spw, restfreq, nchan in zip(spws.split(","), restfreqs, nchans):
            ...
            imagename = os.path.join(self.outdir, imagename)
            self.logger.info(
                "Dirty imaging spw {0} (restfreq: {1})...".format(
                    spw, restfreq
                )
            )
            casa.tclean(
            ...
            if not os.path.exists(pbimage):
                raise ValueError(
                    "Could not find {0}. Did you dirty MFS first?".format(
                        pbimage
                    )
                )
            casa.impbcor(
    ...
    def channel_clean_spws(self, spws, spwtype):
        ...
        for spw, restfreq, nchan in zip(spws.split(","), restfreqs, nchans):
            ...
            if self.uvtaper:
                ...
                mask = "{0}.spw{1}.{2}.mfs.mask".format(
                    self.field, spw, self.stokes
                )
            imagename = os.path.join(self.outdir, imagename)
    ...
    def contplot(self):
        ...
        spws = ["cont"] + natural_sort(
            self.cont_spws.split(",") + self.line_spws.split(",")
        )
        for spw in spws:
            if spw != "cont":
            ...
            if self.uvtaper:
                ...
                titles = [
                    "{0} - {1} - Taper/Dirty".format(self.field, spw),
                ...
                titles = [
                    "{0} - {1} - Dirty".format(self.field, spw),
            ...
            for ind, stokes in enumerate(self.stokes):
                ...
                for fitsfile, maskfile, title, label, vlim in zip(
                    ...
                    if maskfile != "":
                        maskhdu = fits.open(
                            os.path.join(self.outdir, maskfile)
                        )[0]
                        contour = ax.contour(
                            maskhdu.data[ind, 0],
                            levels=[0.5],
                    ...
                    #
                    fname = fitsfile.replace(
                        ".fits", ".{0}.pdf".format(stokes)
                    )
                    fig.savefig(
                        os.path.join(self.outdir, fname), bbox_inches="tight"
                    )
                    plt.close(fig)
                    plt.ion()
    ...
    def lineplot(self):
        ...
        for spw in self.line_spws.split(","):
            ...
            if self.uvtaper:
                ...
                maskfile = (
                    "{0}.spw{1}.{2}.channel.uvtaper.clean.mask.fits".format(
                        self.field, spw, self.stokes
                    )
                )
                titles = [
        ...
        # need to fix filenames so LaTeX doesn't complain
        goodplots = [
            "{" + fn.replace(".pdf", "") + "}.pdf" for fn in goodplots
        ]
        self.logger.info("Generating PDF...")
